...
export async function getProductionReadiness() {
  ...
  const requiredChecks = productionMode ? checks : checks.filter((check) => check.label !== "day1_brain_concurrency" && check.label !== "distributed_queue" && check.label !== "persistent_memory");
