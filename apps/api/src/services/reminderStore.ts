...
export function reminderStoreStatus() {
  return {
    persistence: supabase ? "supabase" : "memory",
    active: true,
    launchSafe: Boolean(supabase),
    warning: supabase ? null : "In-memory reminders are beta-only on Render because the filesystem/process can restart.",
    localCount: localReminders.length,
    scaleTestCount: scaleTestReminders.length
  };
}
