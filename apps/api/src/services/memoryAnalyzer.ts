...
const explicitSavePattern = /\b(save this|save it|save that|save karo|save kar|journal this|journal it|add to journal|note this|remember this|remember that|remember it|yaad rakh|yaad rakhna|memory mein|memory me|isko save|ye save|isey save|pin this)\b/i;
...
export async function analyzeMemory(content: string): Promise<MemoryAnalysis> {
  ...
  const explicitSaveIntent = explicitSavePattern.test(content);
  const importantAutoMemory = hasAction || hasGoal || hasEmotion || hasStableProfileSignal || /\b(critical|urgent|deadline|doctor|health|money|family|relationship|work|job|career|trading|fitness|sleep|developer|coding|swift|vibe coding|aura app|aura project|build aura)\b/i.test(content);
