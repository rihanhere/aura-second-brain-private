...
import { classifyMemoryIntent, hasNegatedMemoryCommand, isMemoryCapabilityQuestion, type MemoryIntent } from "./memoryIntentRouter.js";
...
const explicitSaveTurnPattern = /\b(save this|save it|save that|save karo|save kar|journal this|journal it|jarnel this|jarnal this|journal karo|journal kar|add to journal|note this|remember this|remember that|remember it|yaad rakh|yaad rakhna|memory mein|memory me|isko save|ye save|isey save|pin this)\b/i;
...
function relationshipRecallLabel(query: string) {
  ...
  const namedPerson = query.match(/\b(?:about|remember about|know about)\s+([A-Z][a-z]{2,})\b/);
  if (namedPerson?.[1] && !/^(AURA|Aura|Rihan|Me|My)$/i.test(namedPerson[1])) {
    return namedPerson[1].toLowerCase();
  }
  return null;
}
...
function cleanGoalPhrase(content: string) {
  const clean = compactText(content, 220)
    ...
    .replace(/^my\s+(?:main\s+)?goal\s+is\s+/i, "")
    .replace(/^main\s+goal\s+is\s+/i, "")
    .replace(/^i\s+(?:want|need|plan|am trying|try|m trying|would like)\s+to\s+/i, "to ")
    .replace(/^learning\s+/i, "to learn ")
    .replace(/\s+/g, " ")
    .replace(/[?.!]+$/g, "")
    .trim();
...
function toSecondPersonFact(text: string, maxLength = 180) {
  return compactText(text, maxLength)
    ...
    .replace(/\bI plan\b/gi, "you plan")
    .replace(/\bI prefer\b/gi, "you prefer")
    .replace(/\bI like\b/gi, "you like")
    .replace(/\bI hate\b/gi, "you hate")
    .replace(/\bI learn\b/gi, "you learn")
    .replace(/\bI code\b/gi, "you code")
    .replace(/\bI often\b/gi, "you often")
    .replace(/\bI\b/g, "you")
...
async function buildGoalRecallArchive(input: { userId: string; timezone: string; excludeIds?: string[] }) {
  ...
  const archiveTurns = (await listConversationTurns(input.userId, 800))
    ...
    .filter((turn) => /\b(my (?:main\s+)?goal is|main goal is|i want to|i need to|i am trying to|i'm trying to|trying to become|become a developer|learn coding|learning swing trading|swing trading|overtrading|build aura|developer|vibe coding|practice guitar|upload covers|get fit|walk every morning)\b/i.test(turn.content))
  ...
  const seen = new Set([main.toLowerCase().replace(/[^a-z0-9]+/g, " ").trim()]);
  const uniqueSupporting = supporting.filter((phrase) => {
    const key = phrase.toLowerCase().replace(/[^a-z0-9]+/g, " ").trim();
    if (!key || seen.has(key)) return false;
    seen.add(key);
    return true;
  });
  const answerHint = uniqueSupporting.length === 0
    ? `Your goal is ${main}.`
    : `Your main goal is ${main}. I also remember: ${uniqueSupporting.join("; ")}.`;
...
async function buildProfileRecallArchive(input: { userId: string; timezone: string; excludeIds?: string[] }) {
  ...
  const rawFacts = [
    ...
    ...atoms.map((atom) => {
      ...
      return toSecondPersonFact(atom.text);
    }),
...
function buildExplicitSaveAnswer(turns: ConversationTurn[], timezone: string) {
  const saveTurns = turns
    .filter((turn) => turn.role === "user" && explicitSaveTurnPattern.test(turn.content) && !hasNegatedMemoryCommand(turn.content) && !isMemoryCapabilityQuestion(turn.content) && turn.content.trim().length > 0)
...
async function buildFirstConversationRecall(input: { userId: string; timezone: string; excludeIds?: string[] }) {
  const excludeIds = new Set(input.excludeIds ?? []);
  const turns = (await listConversationTurns(input.userId, MAX_LOCAL_ARCHIVE))
  ...
  if (!first) {
    return {
      answerHint: "I don't have an earlier conversation archived for you yet.",
      context: "First conversation recall requested, but no prior user turns were found."
    };
  }

  const when = formatInTimeZone(new Date(first.created_at), input.timezone || first.timezone || "UTC", "yyyy-MM-dd HH:mm");
  return {
    answerHint: `We first started talking on ${when}. The first message I have archived from you was: "${compactText(first.content, 180)}"`,
    context: `First archived user turn:\n- [${when}] User: ${first.content}`
  };
}
...
function isDidITellRecallQuestion(query: string) {
  return /\b(did i|have i)\b.{0,40}\b(tell|told|say|said|mention|mentioned)\b/i.test(query)
    || /\b(kya)\b.{0,30}\b(maine|mene|mainne)\b.{0,80}\b(bataya|bola|kaha)\b/i.test(query)
    || /\b(maine|mene|mainne)\b.{0,80}\b(pehle|pahle|kabhi|earlier|before)\b.{0,80}\b(bataya|bola|kaha)\b/i.test(query)
    || /(क्या).{0,30}(मैंने|मैने).{0,90}(बताया|बोला|कहा|पूछा)/i.test(query)
    || /(मैंने|मैने).{0,90}(पहले|कभी).{0,90}(बताया|बोला|कहा)/i.test(query);
}

function cleanDidITellQuery(query: string) {
  if (/(ऐप|एप|app|application)/i.test(query)) return "app aura assistant developing building";
  if (/(डेवलपर|developer|coding|code)/i.test(query)) return "developer coding app aura";
  if (/(दोस्त|friend)/i.test(query)) return "friend";
  return query
    .replace(/\b(did i|have i|can you remember|do you remember|when|what|kya|maine|mene|mainne|tumhe|tujhe|pehle|pahle|kabhi|earlier|before|tell|told|say|said|mention|mentioned|bataya|bola|kaha|ki|that|about|you|to|the)\b/gi, " ")
    .replace(/(क्या|मैंने|मैने|तुम्हें|तुझे|आपको|पहले|कभी|बताया|बोला|कहा|कि|मैं|एक|पे|पर|काम|कर|रहा|हूँ|है|था)/gi, " ")
    .replace(/[?.!,;:]+/g, " ")
    .replace(/\s+/g, " ")
    .trim();
}

function buildDidITellAnswer(query: string, turns: ConversationTurn[], timezone: string) {
  const topic = cleanDidITellQuery(query);
  ...
  const userTurns = turns
    ...
    .map((turn) => ({ turn, score: terms.length ? keywordScore(topic, turn) : keywordScore(query, turn) }))
    .filter((item) => item.score > 0)
    .sort((a, b) => b.score - a.score || new Date(a.turn.created_at).getTime() - new Date(b.turn.created_at).getTime());

  const match = userTurns[0]?.turn;
  if (!match) {
    return {
      answerHint: topic
        ? `I can't find a clear memory of you telling me about ${topic}. I can search deeper if you want.`
        : "I can't find a clear memory of you telling me that. I can search deeper if you want.",
      context: `Did-I-tell recall requested for "${topic || query}", but no matching user turns were found.`
    };
  }

  const when = formatInTimeZone(new Date(match.created_at), timezone || match.timezone || "UTC", "yyyy-MM-dd HH:mm");
  return {
    answerHint: `Yes. You told me on ${when}: "${compactText(match.content, 220)}"`,
    context: `Direct user-history match:\n- [${when}] User: ${match.content}`
  };
}
...
function relationshipFactFromTurn(turn: ConversationTurn, label: string) {
  ...
  if (label === "mother" && /\bmedicine\b/i.test(content)) return "she reminds you to take medicine";
  if (label === "friend") {
    return compactText(content
      .replace(/\bmy friend\s+([A-Z][a-z]+)\b/i, "$1")
      .replace(/\bhelps me\b/i, "helps you")
      .replace(/\bmy\b/gi, "your")
      .replace(/\bour\b/gi, "your"), 180).replace(/[?.!]+$/g, "");
  }
  if (!relationshipAliases[label]) {
    const display = label.charAt(0).toUpperCase() + label.slice(1);
    return compactText(content
      .replace(new RegExp(`\\bmy\\s+(?:friend|bandmate|partner|coworker|cofounder|co-founder)\\s+${display}\\b`, "i"), display)
      .replace(/\bmy\b/gi, "your")
      .replace(/\bour\b/gi, "your"), 180).replace(/[?.!]+$/g, "");
  }
  return compactText(content.replace(/\bmy\b/gi, "your").replace(/\bour\b/gi, "your"), 180).replace(/[?.!]+$/g, "");
}
...
function buildRelationshipRecallAnswer(label: string, turns: ConversationTurn[]) {
  ...
  if (!facts.length) {
    const subject = relationshipAliases[label] ? `your ${label}` : label.charAt(0).toUpperCase() + label.slice(1);
    return `I don't have a clear memory about ${subject} yet.`;
  }

  const subject = relationshipAliases[label] ? `your ${label}` : label.charAt(0).toUpperCase() + label.slice(1);
  if (facts.length === 1) {
    return `About ${subject}, I remember that ${facts[0]}.`;
  }

  return `About ${subject}, I remember that ${facts.slice(0, -1).join(", ")}, and ${facts[facts.length - 1]}.`;
}
...
export async function buildArchiveContext(input: {
  ...
  if (memoryIntent === "first_conversation_recall") {
    const firstConversation = await buildFirstConversationRecall(input);
    return {
      recall: { isRecall: true, label: "first conversation", topicQuery: "first conversation", kind: memoryIntent },
      answerHint: firstConversation.answerHint,
      context: firstConversation.context
    };
  }

  if (isDidITellRecallQuestion(input.query)) {
    const turns = (await listConversationTurns(input.userId, 1000))
      .filter((turn) => !(input.excludeIds ?? []).includes(turn.id));
    const direct = buildDidITellAnswer(input.query, turns, input.timezone);
    return {
      recall: { isRecall: true, label: "direct user-history recall", topicQuery: cleanDidITellQuery(input.query), kind: "user_history_recall" },
      answerHint: direct.answerHint,
      context: direct.context
    };
  }

  if (memoryIntent === "memory_capability_question") {
    return {
      recall: { isRecall: false, label: "memory capability", topicQuery: "", kind: memoryIntent },
      answerHint: "",
      context: ""
    };
  }
